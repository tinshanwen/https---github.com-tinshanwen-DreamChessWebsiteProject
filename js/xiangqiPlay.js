...
// 1.4 Real Time Game Element
let gameId = 0; 
let currentFEN = 'rheagaehr/9/1c5c1/s1s1s1s1s/9/9/S1S1S1S1S/1C5C1/9/RHEAGAEHR r'; // MUST Initialise
let thisPlayerColor = "";
let whiteTime = 0;
let blackTime = 0;
let result = ""
...
function updateTimer() { 
  ...
  if(playerTurn == "red") whiteTime-=0.05;
  ...
  timeDisplay.textContent = `Red: ${whiteMinutes.toString().padStart(2, '0')}:${whiteSeconds.toString().padStart(2, '0')} - Black: ${blackMinutes.toString().padStart(2, '0')}:${blackSeconds.toString().padStart(2, '0')}`;
...
resignButton.onclick = function() {
  ...
  if (userConfirmed) {
    ...
    result = (thisPlayerColor == "red") ? "black" : "red";
    gameEnded(); 
  }
};
...
function gameEnded() {
  ...
  if (result == "draw") {
    ...
    playerDisplay.textContent = "";
    infoDisplay.textContent = "Red win";
...
function dragDrop(e) {
  ...
  if (checkForStalemate(opponentColor)) {
    // In Xiang Qi Stalemate is consider Checkmate
    result = playerColor; // If it's a checkmate, the current player wins
    gameEnded();
  }
}
